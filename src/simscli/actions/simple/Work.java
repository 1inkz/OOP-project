package simscli.actions.simple;

import simscli.actions.Action;
import simscli.game.GameContext;
import simscli.jobs.Job;
import simscli.sims.Sim;
import simscli.stats.NeedType;
import simscli.stats.SkillType;

public final class Work implements Action {

    private static final String RED = "\u001B[31m";
    private static final String GREEN = "\u001B[32m";
    private static final String RESET = "\u001B[0m";

    private static final int ENERGY_COST = 30;
    private static final int HYGIENE_COST = 15;
    private static final int FUN_COST = 10;

    @Override
    public String name() {
        return "Work";
    }

    @Override
    public String perform(Sim sim, GameContext ctx) {
        if (!sim.isAlive()) {
            return RED + sim.getName() + " is dead and cannot work!" + RESET;
        }

        Job job = sim.getJob();
        if (!job.canWork()) {
            return RED + sim.getName() + " is jobless and cannot work!" + RESET;
        }

        int currentEnergy = sim.getNeeds().get(NeedType.ENERGY);
        if (currentEnergy < ENERGY_COST) {
            return RED + sim.getName() + " is too tired to work! (Need at least " + ENERGY_COST + " energy)" + RESET;
        }

        sim.getNeeds().add(NeedType.ENERGY, -ENERGY_COST);
        sim.getNeeds().add(NeedType.HYGIENE, -HYGIENE_COST);
        sim.getNeeds().add(NeedType.FUN, -FUN_COST);

        int previousJobLevel = sim.getJobLevel();
        double baseSalary = job.salary(previousJobLevel);

        int primaryAverage = sim.getSkills().average(job.primarySkills());
        int workEthic = sim.getSkillLevel(SkillType.WORK_ETHIC);

        double salaryMultiplier = 1.0
                + (primaryAverage * 0.003)   // up to +30%
                + (workEthic * 0.002);       // up to +20%

        int earned = (int) Math.round(baseSalary * salaryMultiplier);
        sim.earnSimcoin(earned);

        grantWorkSkillProgress(sim, job);

        sim.setJobLevel(previousJobLevel + 1);
        int newJobLevel = sim.getJobLevel();

        String levelUpMsg = "";
        if (newJobLevel > previousJobLevel) {
            levelUpMsg = " Promoted to Level " + newJobLevel + "!";
        }

        return GREEN
                + sim.getName()
                + " worked as a "
                + job.name()
                + " and earned "
                + earned
                + " Simcoin! (Base: "
                + (int) baseSalary
                + ", Skill Bonus: x"
                + String.format("%.2f", salaryMultiplier)
                + ")"
                + levelUpMsg
                + RESET;
    }

    private void grantWorkSkillProgress(Sim sim, Job job) {
        for (SkillType type : job.primarySkills()) {
            sim.gainSkill(type, 4);
        }
        sim.gainSkill(SkillType.WORK_ETHIC, 2);
    }
}
